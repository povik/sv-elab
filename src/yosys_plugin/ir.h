...
public:
	...
	Const(const std::vector<Trit> &bits)
	{
		...
		for (auto t : bits)
			states.push_back(t.raw_);
		raw_ = RTLIL::Const(std::move(states));
	}

	static Const from_rtlil(RTLIL::Const c) { return Const(std::move(c)); }

	RTLIL::Const to_rtlil() const { return raw_; }
