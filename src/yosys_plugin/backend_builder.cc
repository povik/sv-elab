...
ir::Value BackendGraphBuilder::Shiftx(
...
ir::Value BackendGraphBuilder::Unop(
		ast::UnaryOperator op, ir::Value a, bool a_signed, uint64_t y_width)
{
	bool invert = false;
	RTLIL::IdString rtlil_op;
...
#undef OP
		if (have_const) {
			if (invert) {
				const_result = RTLIL::const_logic_not(
						const_result.as_const().to_rtlil(), {}, false, false, y_width);
			}
			return const_result;
		}
	}
